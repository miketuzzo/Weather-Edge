...
# Reuse one HTTP session (faster than new TCP/TLS every call)
_HTTP = requests.Session()

# Tiny TTL caches to reduce repeat network calls within Streamlit reruns
_TTL_CACHE = {}  # key -> (expires_epoch, value)

def _ttl_get(key, default=None):
...
def _ttl_set(key, val, ttl_s: int):
    _TTL_CACHE[key] = (time.time() + ttl_s, val)

# Cache loaded private key so we don't re-parse it every request
_CACHED_PRIVATE_KEY = None
_CACHED_PRIVATE_KEY_SOURCE = None  # ('pem', hash) or ('path', path)
...
def nws_get_json(url: str) -> dict:
    ...
    r.raise_for_status()
    data = r.json()
    _ttl_set(("nws", url), data, ttl_s=60)
    return data
...
def load_private_key(path: str):
    ...
    global _CACHED_PRIVATE_KEY, _CACHED_PRIVATE_KEY_SOURCE

    # Streamlit Cloud: private key comes from secrets/env as PEM text
    pem = os.getenv("KALSHI_PRIVATE_KEY_PEM", "").strip()
    if pem:
        src = ("pem", hash(pem))
        if _CACHED_PRIVATE_KEY is not None and _CACHED_PRIVATE_KEY_SOURCE == src:
            return _CACHED_PRIVATE_KEY
        _CACHED_PRIVATE_KEY = serialization.load_pem_private_key(pem.encode("utf-8"), password=None)
        _CACHED_PRIVATE_KEY_SOURCE = src
        return _CACHED_PRIVATE_KEY
...
def kalshi_get(path: str, params=None) -> dict:
    ...
    r = _HTTP.get(BASE_URL + path, headers=headers, params=params, timeout=12)
    r.raise_for_status()
    data = r.json()
    # Cache by request signature (path + params). NOTE: a previous version
    # mistakenly referenced an undefined variable `url`, which caused
    # NameError and broke all market fetches.
    key = ("kalshi", path, tuple(sorted((params or {}).items())))
    _ttl_set(key, data, ttl_s=30)
    return data
